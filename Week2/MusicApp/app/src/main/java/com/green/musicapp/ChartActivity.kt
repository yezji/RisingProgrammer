...
class ChartActivity : AppCompatActivity() {
    ...
    fun loadData() : MutableList<ItemSongOfChart> {
        val data:MutableList<ItemSongOfChart> = mutableListOf()

        // TODO : rid 인것을 db걸로 바꾸기
        for (idx in 1..19) {
            val albumRid:Int = resources.getIdentifier("img_album_$idx", "albumRid", packageName)
            val nameRid:Int = resources.getIdentifier("chart_song_name_$idx", "nameRid", packageName)
            val artistRid:Int = resources.getIdentifier("chart_song_artist_$idx", "artistRid", packageName)
            data.add(ItemSongOfChart(albumRid, nameRid, artistRid))
        }
